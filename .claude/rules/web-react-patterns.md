...
- **A ordem dos imports é do lint, não da mão.** Pacotes (react na frente) → alias → relativos → `.`. Dentro do bloco relativo: `../` antes de `./`, e o namespace desce para o fim do seu bloco — primeiro o que vem por nome, depois `* as C` e por último `* as S`. `yarn lint:fix` arruma; não vale reordenar à mão contra a regra.
