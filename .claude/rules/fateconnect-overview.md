...
- **Front:** `FateConnect/Web` — React + Vite, MUI + Emotion, design system local em `src/design-system/` (barrels `@design-system` e `@design-system/icons`), variáveis em `.env*` (`VITE_*`). Padrões: **`.claude/rules/fateconnect-web-react.md`**.
