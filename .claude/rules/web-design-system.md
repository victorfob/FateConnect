...
- **Ação secundária usa `variant="soft"`**, a variante de contorno neutro declarada no tema. ⛔ Não componha uma aparência nova no ponto de uso — `variant="outlined" color="inherit"` apareceu em dois PRs no mesmo dia, escolhido duas vezes sem ninguém combinar. Falta variante para o que você precisa: **declare no tema**, como se faz com token.
  ...
  ⚠️ **O rodapé de diálogo é a exceção, e ela é medida.** Ali as duas ações são `contained` — `primary` na neutra e `secondary` na que confirma —, porque o `soft` carrega borda de 1px e sai **6px mais alto** que a vizinha, desalinhando o par. Foi assim que o botão de limpar do filtro nasceu errado: a regra acima não falava de rodapé, e o `soft` parecia a leitura certa.
...
- ⛔ **E a lista de pares fixos é escrita à mão, então par que ninguém acrescenta ninguém mede.** O `boundPairs` enumerava o par do cromo e o do `secondary`, e **não** o de `primary.contrastText` sobre `primary.main` — o par de três botões do produto, onde o branco daria 4,06:1 e reprovaria. A geração automática cobre `contentColours` contra as superfícies; `contrastText` novo na paleta obriga a acrescentar o par ali à mão.
- ⛔ **E há par que a paleta não declara em lugar nenhum: ele nasce da sobrescrita de CSS.** Aí não adianta reler a paleta procurando o que falta — o par não está lá. Regra que pinta `color` sobre um conjunto amplo de filhos (`& .MuiButton-root`) casa junto o filho que traz o **próprio** fundo, e o que a tela desenha passa a ser a cor de uma paleta sobre o fundo de outra. Nenhum teste que enumere a paleta alcança isso; quem acusou foi o Lighthouse, renderizando a página. ⚠️ O tell é o mesmo arquivo já excluir aquela variante em **outra** regra — no cabeçalho o `:not(.MuiButton-contained)` do realce de hover prova que a exceção era conhecida, e ela ficou faltando na regra de cor.
...
- ⛔ **Contraste é luminância, e saturação sai de graça.** Precisando de mais contraste sobre um fundo escuro, o reflexo é clarear — e clarear "lavando" a cor é escolha, não consequência. Na #318 o vermelho da marca sobre o cromo estava em `#E7867E`: 3,05:1 com **69%** de saturação, e a devolução foi *"ta mto opaco"*. No mesmo nível de claridade existiam tons com **100%** de saturação — a busca devolveu `#FF7575` a 3,03:1. Antes de aceitar uma cor lavada, varra a matiz mantendo a luminância: o que a fórmula da WCAG mede é ela, não a saturação.
- ⚠️ **Logotipo é isento do mínimo da WCAG**, e por isso não entra em `contentColours` nem em `nonTextColours` — com 2,47:1 a marca reprovaria uma lista que ela não deve cumprir, e limite que não se exige não fica no teste fingindo que exige. O que se guarda ali é a decisão que existe: a marca sobre o cromo nunca é menos legível que o vermelho de botão seria no mesmo lugar.
